...
export function matchTypes(main: OperationType, tx: { [id: number]: number }, a: number, b: number): void {
    ...
    const typeA = main.types[a];
    const typeB = main.types[b];
...
function matchArity(main: OperationType,
                    tx: { [id: number]: number },
                    arityA: null | TypeArity,
                    arityB: null | TypeArity): null | TypeArity {
    if (arityA === null) {
        return arityB;
    } else if (arityB === null) {
        return arityA;
...
function matchArrays(main: OperationType,
                     tx: { [id: number]: number },
                     newType: OperandType,
                     arrayA: number, arrayB: number): void {
    matchTypes(main, tx, arrayA, arrayB);
    newType.array = arrayA;
}

function matchTuples(main: OperationType,
                     tx: { [id: number]: number },
                     newType: OperandType,
                     tupleA: number[],
                     tupleB: number[]): void {
...
function matchWrapped(main: OperationType,
                      tx: { [id: number]: number },
                      newType: OperandType,
                      wrappedA: undefined | TypeArity,
                      wrappedB: undefined | TypeArity): void {
